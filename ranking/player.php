...
$rank = $rankrow['rank'];
$arr_kills = array();
$arr_kills['普通感染者'] = array($row['kill_infected'], "Common Infected killed");
...
$arr_survivor_awards['地图完成总数:'] = array($row['award_campaigns'], "Campaigns Completed");
$arr_survivor_awards['所有幸存者到达安全门总次数:'] = array($row['award_allinsafehouse'], "Safe Houses Reached with No Deaths");
$arr_survivor_awards['灌油总桶数:'] = array($row['award_gascans_poured'], "Successfully poured Gas Canisters");
$arr_survivor_awards['弹药包部署总数:'] = array($row['award_upgrades_added'], "Ammo Upgrades Deployed");
$arr_survivor_awards['肾上腺素赠与总数:'] = array($row['award_adrenaline'], "Adrenalines given to another Survivor");
$arr_survivor_awards['药丸赠与总数:'] = array($row['award_pills'], "Pills given to another Survivor");
$arr_survivor_awards['医疗包赠与总数:'] = array($row['award_medkit'], "Healed another Survivor");
$arr_survivor_awards['拉起队友总数:'] = array($row['award_revive'], "Helped Incapacitated Survivors");
$arr_survivor_awards['电起队友总数:'] = array($row['award_defib'], "Dead Survivors brought back to life");
$arr_survivor_awards['营救队友总数:'] = array($row['award_rescue'], "Rescued Survivors from rescue rooms");
$arr_survivor_awards['保护队友总数:'] = array($row['award_protect'], "Protected another Survivor from a common Infected");
$arr_survivor_awards['从Smokers救下队友总次数:'] = array($row['award_smoker'], "Saved a Survivor from Smokers");
$arr_survivor_awards['从Hunters救下队友总次数:'] = array($row['award_hunter'], "Saved a Survivor from Hunters");
$arr_survivor_awards['从Chargers救下队友总次数:'] = array($row['award_charger'], "Saved a Survivor from Chargers");
$arr_survivor_awards['从Jockeys救下队友总次数:'] = array($row['award_jockey'], "Saved a Survivor from Jockeys");
$arr_survivor_awards['近战刀牛总数:'] = array($row['award_matador'], "Killed a Charging Charger with a melee weapon");
$arr_survivor_awards['和队友杀死Tank数量:'] = array($row['award_tankkill'], "Killed Tanks");
$arr_survivor_awards['无人死亡杀死Tank数量:'] = array($row['award_tankkillnodeaths'], "Killed Tanks with No Deaths");
$arr_survivor_awards['秒妹数量:'] = array($row['award_witchcrowned'], "Successfully Crowned Witches");
$arr_infected_awards = array();
$arr_infected_awards['造成生还者全部死亡次数:'] = array($row['award_infected_win'], "All Survivors dead");
$arr_infected_awards['造成生还者倒地次数:'] = array($row['award_survivor_down'], "Survivors incapacitated");
$arr_infected_awards['使用Boomer完美致盲次数:'] = array($row['award_perfect_blindness'], "All Survivors blinded");
$arr_infected_awards['完美扑次数:'] = array($row['award_pounce_perfect'], "Perfect Hunter pounces");
$arr_infected_awards['成功扑次数:'] = array($row['award_pounce_nice'], "Very good Hunter pounces");
$arr_infected_awards['使用Tank造成大量伤害次数:'] = array($row['award_bulldozer'], "Dealing massive damage to Survivor");
$arr_infected_awards['牛一次撞多人次数:'] = array($row['award_scatteringram'], "Charged a Scattering Ram at a group of survivors");
$arr_demerits = array();
$arr_demerits['黑枪次数:'] = array($row['award_friendlyfire'], "Survivors harmed");
$arr_demerits['杀死队友次数:'] = array($row['award_teamkill'], "Survivors killed");
$arr_demerits['击倒队友次数:'] = array($row['award_fincap'], "Survivors incapacitated");
$arr_demerits['放弃队友次数:'] = array($row['award_left4dead'], "Survivors left to die");
$arr_demerits['让感染者进安全门次数:'] = array($row['award_letinsafehouse'], "Infected let in safe room");
$arr_demerits['惊扰Witch次数:'] = array($row['award_witchdisturb'], "Witches disturbed");
