...
if($type == "" || $type == "coop" || $type == "realism" || $type == "versus" || $type == "survival" || $type == "scavenge" || $type == "realismversus" || $type == "mutations") {
	...
	$tpl->set("title", "- 排名榜"); // Window title
	$tpl->set("page_heading", "排名榜 " . $typelabel); // Page header
	$sort = "";
	$playtime = "";
		...
		if ($row['players_count'] > 0) {
			...
			if (mysql_error()) {$arr_players[] = "<p><b>MySQL Error:</b> " . mysql_error() . "</p>";}
				...
				while ($row = mysql_fetch_array($result)) {
					...
					$line .= "<tr onclick=\"window.location='player.php?steamid=" . $row['steamid']."'\" style=\"cursor:pointer\"><td data-title=\"排名:\">" . number_format($i) . "</td><td data-title=\"玩家:\">" . htmlentities($row['name'], ENT_COMPAT, "UTF-8") . "</td>";
					$line .= "<td data-title=\"分数:\">" . number_format($row['real_points']) . "</td>";
					$line .= "<td data-title=\"游玩时间:\">" . formatage($row['real_playtime'] * 60) . "</td>";
	...
	else { $arr_players[] = "<th colspan=\"6\" class=\"text-center\">没有玩家数据.</th>\n";}
	$stats->set("players", $arr_players);
	$output = $stats->fetch("" . $templatefiles['ranking_output.tpl']);
	}
}
else {$tpl->set("title", "排名榜"); $tpl->set("page_heading", "排名榜");$output = "<meta http-equiv=\"refresh\" content=\"1; URL=index.php?type=coop\"><br /><center>You will be forwarded ...</center>";}
...
if ($result && mysql_num_rows($result) > 0)
{
	while ($row = mysql_fetch_array($result)) {
		...
		{
			if ($top3_glow == "enabled") { $playername = "<div class=\"tablex\"><div id=\"steamprofile_avatar\" class=\"text-left steamprofile\" title=" . $row['steamid'] . "></div><div id=\"steamprofile_avatar_top_legendary\"></div><div id=\"steamprofile_avatar_fix\"></div><div class=\"cellx\"><div style=\"position:relative; overflow: hidden;\" class=\"text-left\">&nbsp;玩家: " . $playername . "<div class=\"text-left\">&nbsp;分数: " . gettotalpoints($row) . "</div><div class=\"text-left\">&nbsp;Glow Reward: <font color=\"#feed77\">Legendary</font></div></div></div></div>"; }
				else { $playername = "<div class=\"tablex\"><div id=\"steamprofile_avatar\" class=\"text-left steamprofile\" title=" . $row['steamid'] . "></div><div id=\"steamprofile_avatar_top_legendary\"></div><div id=\"steamprofile_avatar_fix\"></div><div class=\"cellx\"><div style=\"position:relative; overflow: hidden;\" class=\"text-left\">&nbsp;玩家: " . $playername . "<div class=\"text-left\">&nbsp;分数: " . gettotalpoints($row) . "</div></div></div></div>"; }
		}
...
if ($result && mysql_num_rows($result) > 0){
	while ($row = mysql_fetch_array($result)) {
		...
		if ($avatarimg){$playername = "<table border=0 cellspacing=0 cellpadding=0 class=\"top10\"><tr><td>&nbsp;</td><td>" . $avatarimg . "</td>" . ($playerheadline ? "" : "<td>&nbsp;</td>") . "<td>" . $playername . "</td></tr></table>";}
		if (!$playerheadline && !$avatarimg){
			if ($top3_glow == "enabled") { $playername = "<div class=\"tablex\"><div id=\"steamprofile_avatar\" class=\"text-left steamprofile\" title=" . $row['steamid'] . "></div><div id=\"steamprofile_avatar_top_epic\"></div><div id=\"steamprofile_avatar_fix\"></div><div class=\"cellx\"><div style=\"position:relative; overflow: hidden;\" class=\"text-left\">&nbsp;玩家: " . $playername . "<div class=\"text-left\">&nbsp;分数: " . gettotalpoints($row) . "</div><div class=\"text-left\">&nbsp;Glow Reward: <font color=\"#e300fd\">Epic</font></div></div></div></div>"; }
...
if ($result && mysql_num_rows($result) > 0){
	while ($row = mysql_fetch_array($result)) {
		...
		$playername = ($showplayerflags ? "" : "") . "" . $name . "";
		if (!$playerheadline && !$avatarimg){
			if ($top3_glow == "enabled") { $playername = "<div class=\"tablex\"><div id=\"steamprofile_avatar\" class=\"text-left steamprofile\" title=" . $row['steamid'] . "></div><div id=\"steamprofile_avatar_top_rare\"></div><div id=\"steamprofile_avatar_fix\"></div><div class=\"cellx\"><div style=\"position:relative; overflow: hidden;\" class=\"text-left\">&nbsp;玩家: " . $playername . "<div class=\"text-left\">&nbsp;分数: " . gettotalpoints($row) . "</div><div class=\"text-left\">&nbsp;Glow Reward: <font color=\"#025bea\">Rare</font></div></div></div></div>"; }
